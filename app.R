...
library(dashboardthemes)  # temas predefinidos
...
limpiar_datos <- function(archivo, nombre_profesor) {
  ...
  for (i in seq_along(lineas)) {
    if (grepl("^\\d{2}:\\d{2}:\\d{2}\\.\\d{3} -->", lineas[i])) next
      ...
      comentario_temporal <- paste(comentario_temporal,
                                   lineas[i],
                                   sep=" ")
    }
  }
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName="acumulativo",
              ...
              fluidRow(
                box(title="Resumen Acumulativo",
                    status="success", solidHeader=TRUE, collapsible=TRUE,
                    width=12,
                    DTOutput("acumulativo_df"),
                    br(),
      ...
      tabItem(tabName="grafico",
              fluidRow(
                box(title="Controles Gráfico",
                    status="warning", solidHeader=TRUE, collapsible=TRUE,
                    width=6,
                    pickerInput("clase_seleccionada", "🔎 Sesión:",
                                choices = NULL,
                                options = list(`live-search`=TRUE),
                                width="100%"),
                    pickerInput("metrica_seleccionada", "📊 Métrica:",
                                choices = c("Chat"="Participaciones",
                                            "Longitud"="longitud",
                                            "Micrófono"="Microfono",
                                            "Webcam"="Webcam"),
                                width="100%"),
                ...
                box(title="Gráfico de Barras",
                    status="warning", solidHeader=TRUE, collapsible=TRUE,
                    width=6,
                    plotOutput("grafico_barras", height="400px"),
                    br(),
      ...
      tabItem(tabName="normas",
              ...
              fluidRow(
                box(title="Resultados",
                    status="primary", solidHeader=TRUE, collapsible=TRUE,
                    width=12,
                    DTOutput("tabla_calificaciones"),
                    br(),
                    downloadButton("download_norms", "⬇️ Descargar")
                )
              ),
              fluidRow(
                box(title="Distribución de Calificaciones",
                    status="success", solidHeader=TRUE, collapsible=TRUE,
                    width=12,
...
server <- function(input, output, session) {
  ...
  normas_default <- tibble(
    Calification  = 1:10,
    Points_gained = seq(7.5, by=7.5, length.out=10)
  )
  normas_unidas  <- reactiveVal(normas_default)
  
  # 1) Procesar .txt y poblar datos_unidos()
  observeEvent(input$txt_files, {
    ...
    df_int <- df_base %>%
      full_join(uso_mic, by=c("Clase","Nombre")) %>%
      full_join(uso_web, by=c("Clase","Nombre"))
    
    datos_unidos(df_int)
    
    # Actualizar el picker de Sesión
    updatePickerInput(session, "clase_seleccionada",
                      choices = clases,
                      selected = clases[1])
  })
  
  # 2) Reactivos de datos
  datos_originales <- reactive({
    req(nrow(datos_unidos())>0)
    datos_unidos()
  })
  ...
  output$n_sesiones <- renderInfoBox({
    ...
    infoBox("Sesiones", length(unique(datos_originales()$Clase)),
            icon=icon("calendar"), color="purple")
  })
  output$n_estudiantes <- renderInfoBox({
    req(datos_originales())
    infoBox("Estudiantes", length(unique(datos_originales()$Nombre)),
  ...
  output$acumulativo_df <- renderDT({
    datatable(acumulativo(), options=list(pageLength=10, searchHighlight=TRUE))
  })
  ...
  output$grafico_barras <- renderPlot({
    req(datos_originales(), input$clase_seleccionada, input$metrica_seleccionada)
    for(i in seq(0,100, by=25)) {
      updateProgressBar(session, "prog_plot", value=i)
      Sys.sleep(0.02)
    }
    df <- datos_originales() %>% filter(Clase==input$clase_seleccionada)
    ggplot(df, aes(
      x = reorder(Nombre, !!sym(input$metrica_seleccionada)),
      y = !!sym(input$metrica_seleccionada)
    )) +
  ...
  output$download_plot <- downloadHandler(
    filename = function() paste0("Grafico_Barras_",
                                 input$clase_seleccionada, "_",
                                 Sys.Date(), ".jpg"),
  ...
  observeEvent(input$tabla_normas_cell_edit, {
    info <- input$tabla_normas_cell_edit
    df   <- normas_unidas()
    df[info$row, info$col+1] <- as.numeric(info$value)
    normas_unidas(df)
    replaceData(proxy_normas, df, resetPaging=FALSE, rownames=FALSE)
  })
  
  # ---- Calificaciones según normas ----
  tabla_calificada <- reactive({
    req(datos_con_pesos())
    datos_con_pesos() %>%
      replace_na(list(Microfono=0, Webcam=0)) %>%
      group_by(Nombre) %>%
      summarise(
        Chat          = sum(Participaciones, na.rm=TRUE),
      ...
      mutate(
        Calification = {
          vals <- normas_unidas()$Calification[
            normas_unidas()$Points_gained <= Points_gained]
          if (length(vals)==0) 0 else max(vals, na.rm=TRUE)
        }
      )
  })
  output$tabla_calificaciones <- renderDT({
    datatable(tabla_calificada(), options=list(pageLength=10, searchHighlight=TRUE))
  })
  ...
  output$valor_promedio <- renderInfoBox({
    ...
    infoBox("Promedio",
            round(mean(tabla_calificada()$Calification, na.rm=TRUE),2),
